...
```{r ans12}
Game5Time <- NBA1920Final567$game5
Game5Time
```
...
```{r ans13}
game5.date = Game5Time[[1]]
game5.time = Game5Time[[2]]
G5Time <- paste0(game5.date, " 2020 ", game5.time)
G5TimeParsed <- mdy_hm(G5Time)
G5TimeParsed
```
...
```{r ans21}
chNumbers <- factor(chNumbers, levels =
                      c("壹","貳","參","肆","伍","陸","柒","捌","玖"))
chNumbers <- as.integer(chNumbers)
...
```{r ans22}
chNumbersWith0 <- factor(chNumbersWith0, levels =
                      c("零","壹","貳","參","肆","伍","陸","柒","捌","玖"))
chNumbersWith0 <- as.integer(chNumbersWith0)
